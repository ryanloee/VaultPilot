...
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\build-windows-installers.ps1 -Platforms x64 -Version 0.1.1
```

You can also build both `x86` and `x64`:

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\build-windows-installers.ps1 -Platforms x86,x64 -Version 0.1.1
```
...
- `artifacts/velopack/packages/win-x64/VaultPilot-0.1.1-win-x64-full.nupkg`
