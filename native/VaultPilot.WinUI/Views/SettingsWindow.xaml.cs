...
/// <summary>
/// Settings window extracted from MainWindow. Displays provider configuration,
/// auto-wake options, and general preferences. Validates input before saving.
///
/// Unlike the ContentDialog-based SettingsDialog, this Window has its own
/// Save/Cancel buttons and does NOT close itself. The caller (MainWindow)
/// subscribes to <see cref="SettingsSaved"/> and <see cref="SettingsCancelled"/>
/// events to apply settings and close the window respectively.
///
...
{
    // ── Static internals ──────────────────────────────────────────────
    // Use a shared transparent brush for fallback when theme brushes are missing.
    // This is identical to SettingsDialog's behaviour and avoids an extra
    // per-window allocation (it's immutable and never modified).
    ...
    // The window receives a delegate that invokes the Rust backend's
    // `checkProviderConnection` IPC method. This keeps SettingsWindow
    ...
    private static bool IsCanonicalProviderType(string? value) =>
        value is not null &&
        (string.Equals(value, ProviderTypeOpenAi, StringComparison.OrdinalIgnoreCase) ||
         string.Equals(value, ProviderTypeAnthropic, StringComparison.OrdinalIgnoreCase) ||
         string.Equals(value, ProviderTypeOllama, StringComparison.OrdinalIgnoreCase));
    ...
    /// <summary>
    /// Fired after the user clicks Save and all validation passes.
    /// The caller (MainWindow) should apply <see cref="UpdatedSettings"/>
    /// and <see cref="ThemeMode"/>, then close the window.
    /// </summary>
    public event Action? SettingsSaved;

    /// <summary>
    /// Fired when the user clicks Cancel or closes the window via the
    /// title-bar button. The caller should close and discard the window.
    /// </summary>
    public event Action? SettingsCancelled;

    // ── #3120 Keyboard navigation ─────────────────────────────────────
    /// <summary>
    ...
    /// </summary>
    /// <param name="settings">Current application settings to populate the window.</param>
    ...
    /// <param name="checkConnectionAsync">Optional delegate to test a provider connection.</param>
    ...
    private void LoadProviderFields(ProviderConfig p)
    {
        ...
        ProviderTypeBox.SelectedIndex = string.Equals(ptype, ProviderTypeAnthropic, StringComparison.OrdinalIgnoreCase) ? 1
            : string.Equals(ptype, ProviderTypeOllama, StringComparison.OrdinalIgnoreCase) ? 2
            : 0;
    }
    ...
    /// <summary>
    /// Top-level window KeyDown handler (#3120). Implements the keyboard
    ...
    private void OnWindowKeyDown(object sender, KeyRoutedEventArgs e)
    {
        ...
        var menu = Microsoft.UI.Input.InputKeyboardSource.GetKeyStateForCurrentThread(VirtualKey.Menu).HasFlag(Windows.UI.Core.CoreVirtualKeyStates.Down);
    ...
    ///     so the user can immediately type a new query. If the window is
    ///     already empty, fall through to the Window-level handler which
    ///     will close the window.</item>
    ...
    private void OnSearchBoxKeyDown(object sender, KeyRoutedEventArgs e)
    {
        switch (e.Key)
        {
            case Windows.System.VirtualKey.Escape:
                ...
                // else: let the Window-level KeyDown close the window.
                break;
    ...
    /// via OnSettingsSearchTextChanged). Otherwise, cancel/close the window (#3120).
    /// </summary>
    ...
    /// field strings. Shared by OnSaveClick (save validation) and
    ...
    private async void OnTestConnectionClick(object sender, RoutedEventArgs e)
    {
        ...
        // Gather live input values from the window fields.
    ...
    // ──────────────────────────────────────────────
    //  Auto-wake field validation (wired in XAML)
    // ──────────────────────────────────────────────
    ...
    {
        // Next wake label is computed externally; no-op here.
    }

    // ──────────────────────────────────────────────
    //  Save / Cancel action handlers
    // ──────────────────────────────────────────────

    /// <summary>
    /// Validate all fields and build <see cref="UpdatedSettings"/>.
    /// Does NOT close the window — the caller (MainWindow) receives
    /// <see cref="SettingsSaved"/>, applies the result, and closes the window.
    /// </summary>
    private void OnSaveClick(object sender, RoutedEventArgs e)
    {
        ...
        ActionBarStatus.Text = string.Empty;
        ...
        if (!providerValid || !wakeWordValid)
        {
            ...
            ActionBarStatus.Text = "请修正上方的无效字段。";
        ...
        // Notify caller — they apply the settings and close the window.
        SettingsSaved?.Invoke();
    }

    /// <summary>
    /// Cancels editing. Does NOT close the window — fires <see cref="SettingsCancelled"/>
    /// so the caller (MainWindow) can discard and close the window.
    /// </summary>
    ...
    private static Brush GetThemeBrush(string key)
    {
        ...
        System.Diagnostics.Debug.WriteLine($"[SettingsWindow.GetThemeBrush] Missing resource key: '{key}', falling back to Transparent.");
