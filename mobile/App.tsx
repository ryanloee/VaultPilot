...
export default function App() {
  ...
  const initApp = useCallback(async () => {
    ...
    // #3696 — Initialize i18n locale BEFORE the first render. Awaiting here
    // (instead of fire-and-forget after 'ready') guarantees the cold start
    // shows the correct language instead of always defaulting to zh-CN until
    // an unrelated re-render happens. initLocale never throws, but keep the
    // try/catch so a storage/localization failure cannot stall startup.
    try {
      await initLocale();
    } catch (e) {
      console.warn('[App] initLocale:', e);
    }
