// @ts-nocheck
/**
 * Issue #2619: onSubmitEditing triggers send. Isolated file.
 */
...
import { addMessage, getLatestSession } from '../../db';
...
it('onSubmitEditing triggers send', async () => {
  ...
  await act(async () => { fireEvent.changeText(getByTestId('chat-input'), '键盘发送'); });
  fireEvent(getByTestId('chat-input'), 'submitEditing');
  await act(async () => {});
  await waitFor(() => expect(addMessage).toHaveBeenCalledWith('test-session-id', 'user', '键盘发送', undefined));
});
