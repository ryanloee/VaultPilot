// @ts-nocheck
/**
 * Test issue #2619 — Static + early-return send tests.
 * No full async send chain triggered, so no React state leaks.
 */
...
import { addMessage, getLatestSession, createSession } from '../../db';
...
describe('ChatScreen (#2619) static', () => {
  it('disabled when input empty', async () => {
    const { getByTestId } = await renderChat();
    expect(getByTestId('send-btn').props.disabled).toBe(true);
  });

  it('enabled after typing', async () => {
    const { getByTestId } = await renderChat();
    await act(async () => { fireEvent.changeText(getByTestId('chat-input'), '你好'); });
    expect(getByTestId('send-btn').props.disabled).toBe(false);
  });

  it('silent fail when sessionId null', async () => {
    getLatestSession.mockResolvedValue(null);
    createSession.mockRejectedValue(new Error('DB locked'));
    const { getByTestId } = await renderChat();
    await act(async () => { fireEvent.changeText(getByTestId('chat-input'), '测试'); });
