// @ts-nocheck
/**
 * Issue #2619: Stop button appears while streaming. Isolated file.
 * chatWithReconnect mock never resolves, keeping streaming=true.
 */
...
jest.mock('../../api/client', () => ({
  chatWithReconnect: jest.fn(async (_h, onChunk) => {
    onChunk({ content: 'thinking...', done: false });
    await new Promise(() => {}); // never resolves — keeps streaming active
  }),
}));
...
it('shows stop button while streaming, hides send button', async () => {
  const { getByTestId, queryByTestId } = await renderChat();
  await act(async () => { fireEvent.changeText(getByTestId('chat-input'), '慢速'); });
  const sendBtn = getByTestId('send-btn');
  await act(async () => { fireEvent.press(sendBtn); });
  await act(async () => {});
  await waitFor(() => {
    expect(getByTestId('stop-btn')).toBeTruthy();
    expect(queryByTestId('send-btn')).toBeNull();
  });
});
